...
#
# Xbox joystick tilt mode.
# 左摇杆方向代表：希望船向哪个方向倾斜。
# 逻辑改成最直观版本：
# - 推前 = 前面两个电机 IN：FRONT_IN
# - 推后 = 后面两个电机 IN：BACK_IN
# - 推右 = 右边两个电机 IN：RIGHT_IN
# - 推左 = 左边两个电机 IN：LEFT_IN
# - 推右前 = 右前角 IN：RF_IN
# 不再自动发送对边 OUT。
controller_mode = "TILT_IN"

#
# Xbox left stick tilt scheduler.
# 左摇杆方向代表希望船向哪个方向倾斜。
...
def ble_notification_handler(sender, data):
    ...
    This runs in the BLE background thread.
    Do not call Tkinter here.
    """
...
def send_action(command):
    ...
    if send_busy:
        if command == "ALLS":
            ui_log_queue.put("STOP requested while BLE busy")
        else:
    ...
    def on_done(f):
        ...
        try:
            f.result()
            ui_log_queue.put("Sent: " + command)
        except Exception as e:
            ui_log_queue.put("Send error: " + str(e))
...
def joystick_to_weighted_commands(x, y, mode, reverse_mode=False):
    ...
    # Button 13 reverse mode:
    # 注意：这里不是反转摇杆方向，而是反转电机动作。
    # 正常 = 对应方向 IN；按住 button 13 = 对应方向 OUT。

    # 目前 Mac / pygame 标定：
    # 你实测前后係反的，所以这里反转 Y 轴：
    # y > 0 = back direction
    # y < 0 = front direction
    # 你之前实测左右係反的，所以这里保留 X 轴反转：
    # x > 0 = left direction
    # x < 0 = right direction
    forward = max(-y, 0.0)
    back = max(y, 0.0)
    ...
    action_suffix = "_OUT" if reverse_mode else "_IN"

    # 左摇杆现在只负责四个角，不再负责纯前/后/左/右。
    # 所以角区要更大，避免摇杆接近正前/正后/正左/正右时没有反应。
    axis_balance_margin = 0.04
    # corner_threshold 越小，越容易进入角电机控制。
    corner_threshold = 0.015

    # Left stick no longer sends two-motor pair commands.
    # Pair direction control is handled by the D-pad / bottom cross button:
    # D-pad up/down/left/right = FRONT_IN / BACK_IN / LEFT_IN / RIGHT_IN.
    # Left stick is kept for corner/single-direction style control only.

   # Diagonal movement: use the corner motor IN.

    # 斜角区域放大：只要两个方向都有明显分量，就优先进入角控制。

    # 推右前：RF_IN；推左前：LF_IN；推右后：RR_IN；推左后：RL_IN。

    if forward > corner_threshold and left > corner_threshold:
        weights["LF" + action_suffix] = (forward + left) * 1.3
    if forward > corner_threshold and right > corner_threshold:
        weights["RF" + action_suffix] = (forward + right) * 1.3
    if back > corner_threshold and right > corner_threshold:
        weights["RR" + action_suffix] = (back + right) * 1.3
    if back > corner_threshold and left > corner_threshold:
        weights["RL" + action_suffix] = (back + left) * 1.3
...
def poll_xbox_controller():
    ...
    global last_dpad_send_time
    global controller_mode
    global xbox_reverse_mode
    ...
    if controller_enabled and joystick is not None and PYGAME_AVAILABLE:
        try:
            ...
            # Xbox D-pad / cross button.
            # Some controllers report D-pad as a pygame "hat".
            # Some macOS / pygame setups report D-pad as normal buttons.
            # Step 1: support both hat and common button mapping.
            hat_x, hat_y = 0, 0
            ...
            # Xbox D-pad button fallback based on actual test result.
            # User tested buttons:
            # physical 前 / Up    = button 10
            # physical 后 / Down  = button 7
            # physical 左 / Left  = button 8
            # physical 右 / Right = button 9
            # But the actual control direction is fully reversed, so invert all four directions here.
            if joystick.get_numbuttons() > 10:
                dpad_down_pressed = bool(joystick.get_button(10))
            if joystick.get_numbuttons() > 7:
                dpad_up_pressed = bool(joystick.get_button(7))
            if joystick.get_numbuttons() > 8:
                dpad_right_pressed = bool(joystick.get_button(8))
            if joystick.get_numbuttons() > 9:
                dpad_left_pressed = bool(joystick.get_button(9))
            ...
            if dpad_right_pressed:
                hat_x = 1
            elif dpad_left_pressed:
                hat_x = -1

            # Diagnostic: show which controller buttons are currently pressed.
            # This helps identify the real D-pad button numbers on this Mac/controller.
            pressed_buttons = []
            for button_index in range(joystick.get_numbuttons()):
                if joystick.get_button(button_index):
                    pressed_buttons.append(button_index)

            # Button 13 = reverse modifier.
            # 按住 button 13 时，左摇杆单电机/角电机方向反转，D-pad 双电机方向也反转。
            xbox_reverse_mode = joystick.get_numbuttons() > 13 and bool(joystick.get_button(13))

            # 左摇杆方向直接决定哪边 IN。
            # RT/LT 不再参与控制，避免逻辑混乱。
            controller_mode = "TILT_IN"
            xbox_mode_label.config(text="Mode: TILT TOWARD STICK")
            # Show live joystick value in the Xbox panel (with D-pad and pressed buttons for diagnostics).
            try:
                xbox_axis_label.config(text=f"Stick X={x_axis:+.2f}   Y={y_axis:+.2f}   D-pad=({hat_x},{hat_y})   Reverse={xbox_reverse_mode}   Buttons={pressed_buttons}")
            except Exception:
                pass
            ...
            # D-pad priority control.
            # 用 Xbox 摇杆下面的十字键控制两个电机一起倾斜：
            # 上 = FRONT_IN，下 = BACK_IN，右 = RIGHT_IN，左 = LEFT_IN。
            dpad_command = None

            dpad_x = hat_x
            dpad_y = hat_y

            # Button 13 reverse mode reverses motor action, not control direction.
            # Normal: same selected side IN.
            # Hold button 13: same selected side OUT.
            pair_suffix = "_OUT" if xbox_reverse_mode else "_IN"

            if dpad_y > 0:
                dpad_command = "FRONT" + pair_suffix
            elif dpad_y < 0:
                dpad_command = "BACK" + pair_suffix
            elif dpad_x > 0:
                dpad_command = "RIGHT" + pair_suffix
            elif dpad_x < 0:
                dpad_command = "LEFT" + pair_suffix

            if dpad_command is not None:
                if now - last_dpad_send_time > controller_send_interval:
                    send_action(dpad_command)
                    last_dpad_send_time = now
                    last_controller_send_time = now
                    last_controller_command = dpad_command
                root.after(80, poll_xbox_controller)
                return
            ...
            if vector_key != controller_last_vector_key:
                controller_weight_bucket = joystick_to_weighted_commands(x_axis, y_axis, controller_mode, xbox_reverse_mode)
                controller_bucket_index = 0
...
def add_log(text):
    """
    Add one line to the UI log and Terminal.
    This function should only be called from the Tkinter main thread.
    """
    print(text)
    ...
    try:
        ...
        if line_count > MAX_LOG_LINES:
            ...
            log_box.delete("1.0", f"{delete_to}.0")
...
def process_thread_queues():
    ...
    while ble_count < MAX_BLE_MESSAGES_PER_TICK:
        try:
            text = ble_text_queue.get_nowait()
            update_received_text(text)
            ble_count += 1
        except queue.Empty:
            break

    log_count = 0
    while log_count < MAX_LOG_MESSAGES_PER_TICK:
        try:
            text = ui_log_queue.get_nowait()
            add_log(text)
            log_count += 1
        except queue.Empty:
            break

    if not ble_text_queue.empty() or not ui_log_queue.empty():
        root.after(10, process_thread_queues)
    else:
...
# ============================================================
add_log("UI ready. Waiting for BLE connection...")
process_thread_queues()
